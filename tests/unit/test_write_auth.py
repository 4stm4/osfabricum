...
def test_read_endpoints_public_when_auth_enabled(app_with_auth: FastAPI) -> None:
    ...
    # Reads are never auth-blocked: any non-401 status means the request
    # reached the endpoint (200, 404, 422 for a missing query param, or 500 if
    # the DB is not ready) rather than being rejected for missing credentials.
    assert client.get("/v1/distributions").status_code != 401
    assert client.get("/v1/profiles?distribution=test").status_code != 401
    assert client.get("/v1/builds").status_code != 401
...
def test_all_write_endpoints_protected(app_with_auth: FastAPI) -> None:
    ...
    write_endpoints = [
        ...
        ("POST", "/v1/build-drafts", {"distribution": "test", "profile": "test", "board": "test"}),
        ("PATCH", "/v1/build-drafts/test", {"inputs": {}}),
        ("DELETE", "/v1/build-drafts/test", None),
    ]
