...
def upgrade() -> None:
    ...
    if "soc_families" in set(sa.inspect(op.get_bind()).get_table_names()):
        return
    ...
    op.create_table(
        ...
        sa.Column("metadata_json", sa.JSON, nullable=True),
        # Inline so the constraint is created with the table (SQLite has no
        # ALTER ADD CONSTRAINT).
        sa.UniqueConstraint("board_id", "revision", name="uq_board_revision"),
    )
    ...
    op.create_table(
        ...
        sa.Column("board_id", sa.String(36), sa.ForeignKey("boards.id"), nullable=False),
        sa.Column(
            "board_revision_id", sa.String(36), sa.ForeignKey("board_revisions.id"), nullable=True
        ),
    ...
    op.create_index(
        "ix_board_default_toolchains_board_id", "board_default_toolchains", ["board_id"]
    )
